...
function gravity(g, width, height, cropWidth, cropHeight) {
  ...
  let x = (width / 2) - (cropWidth / 2);
  let y = (height / 2) - (cropHeight / 2);
...
function xy(modifiers, width, height, cropWidth, cropHeight) {
  ...
  const dims = gravity(modifiers.gravity, width, height, cropWidth, cropHeight);
...
exports.cropFill = (modifiers, size) => {
  ...
  const crop = xy(modifiers, newWd, newHt, cropWidth, cropHeight);
